...
describe('human CLI output', () => {
  ...
  test('shows ready fix action and human-readable target sizes', () => {
    ...
    expect(output).toContain('Fix readiness   ready');
    expect(output).toContain('Next            npm exec --ignore-scripts ai-dev-maintenance@0.2.5 -- fix --safe --yes');
...
describe('public docs for v0.2.5 UX', () => {
  test('readmes document the short npx path and pinned safe path', async () => {
    ...
    expect(readmes).toContain('npx --yes ai-dev-maintenance@0.2.5');
    expect(readmes).toContain('npm exec --yes --ignore-scripts ai-dev-maintenance@0.2.5 -- doctor --show-paths');
